...
echo 'Testing a post request with nothing in (Expecting code 401)'
...
echo 'Testing a post request with nothing in but a cookie (Expecting code 400)'
DATA=$(curl -si -X POST --cookie "session=TRUE" "$URL/new" | grep 'HTTP/1.0')
if [[ $DATA =~ "400" ]]; then
    echo "✔ $DATA - TEST PASSED MOVING ON."
else
    echo "✖ $DATA - TEST FAILED, PLEASE FIX."
...
DATA=$(curl -si -X POST --cookie "session=TRUE" -d 'qty_wheels=4&algo=steady&flag_pattern=plain&qty_tyres=6' "$URL/new" | grep 'HTTP/1.0')
if [[ $DATA =~ "400" ]]; then
    echo "✔ $DATA - TEST PASSED MOVING ON."
else
    echo "✖ $DATA - TEST FAILED, PLEASE FIX."
...
DATA=$(curl -si -X POST --cookie "session=TRUE" -d 'qty_wheels=foo&power_type=bar&power_units=green&aux_power_type=some&aux_power_units=0&hamster_booster=0&flag_color=#000000&flag_pattern=plain&flag_color_secondary=#000000&tyres=knobbly&qty_tyres=4&armour=None&attack=None&qty_attacks=0&fireproof=0&insulated=0&antibiotic=test&banging=0&algo=offensive' "$URL/new" | grep 'HTTP/1.0')
if [[ $DATA =~ "400" ]]; then
    echo "✔ $DATA - TEST PASSED MOVING ON."
else
    echo "✖ $DATA - TEST FAILED, PLEASE FIX."
...
DATA=$(curl -si -X POST --cookie "session=TRUE" -d 'qty_wheels=4&power_type=petrol&power_units=1&aux_power_type=None&aux_power_units=0&hamster_booster=0&flag_color=#000000&flag_pattern=plain&flag_color_secondary=#000000&tyres=knobbly&qty_tyres=4&armour=None&attack=None&qty_attacks=0&fireproof=0&insulated=0&antibiotic=0&banging=0&algo=offensive' "$URL/new" | grep 'HTTP/1.0')
if [[ $DATA =~ "200" ]]; then
    echo "✔ $DATA - TEST PASSED MOVING ON."
else
    echo "✖ $DATA - TEST FAILED, PLEASE FIX."
...
echo 'Testing a post request with valid data WITHOUT a cookie(Expecting code 401)'
